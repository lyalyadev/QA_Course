...
const gotoTextBoxPage = async (page: Page) => { await page.goto('https://demoqa.com/text-box') }
...
const submit = async (page: Page) => { await page.locator('#submit').click() }

async function fillForm(page: Page, fullName: string, email: string, currentAddress: string, permanentAddress: string ){
...
const resultFullName = async (page: Page) => { const value = await page.locator('p#name').textContent(); return value?.split(":")[1].trim(); }
const resultEmail = async (page: Page) => { const value = await page.locator('p#email').textContent(); return value?.split(":")[1].trim(); }
const resultCurrentAddress = async (page: Page) => { const value = await page.locator('p#currentAddress').textContent(); return value?.split(":")[1].trim(); }
const resultPermanentAddress = async (page: Page) => { const value = await page.locator('p#permanentAddress').textContent(); return value?.split(":")[1].trim(); }


for (const data of testFormData) {
    test(`${data.id} Validate results`, async ({ page }) => {
        await gotoTextBoxPage(page);
        await fillForm(page, data.fullName, data.email, data.currentAddress, data.permanentAddress);
        ...
        for (const key in data)
        {
            ...
            // key = currentAdress "p#currentAdress"
            // value = result[currentAdress]

            // key = permanentAdress "p#permanentAdress"
            // value = result[permanentAdress]
